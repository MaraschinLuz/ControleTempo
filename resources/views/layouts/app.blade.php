...
<body class="bg-slate-100 font-sans text-slate-900 antialiased" x-data="appShell" @keydown.escape.window="menu = false">
<a href="#conteudo-principal" class="sr-only z-[60] rounded-lg bg-cyan-400 px-4 py-2 font-bold text-slate-950 focus:not-sr-only focus:fixed focus:left-4 focus:top-4">Ir para o conteúdo</a>
<div class="min-h-screen lg:flex">
    <div x-cloak x-show="menu" x-transition.opacity class="fixed inset-0 z-30 bg-slate-950/70 backdrop-blur-sm lg:hidden" @click="menu = false"></div>

    <aside
        id="menu-lateral"
        :class="menu ? 'translate-x-0' : '-translate-x-full'"
        class="fixed inset-y-0 left-0 z-40 flex h-screen w-72 flex-col overflow-hidden bg-slate-950 text-white shadow-2xl shadow-slate-950/30 transition-transform duration-300 lg:sticky lg:top-0 lg:translate-x-0"
        aria-label="Menu principal"
    >
        <div class="pointer-events-none absolute -right-20 -top-24 h-56 w-56 rounded-full bg-cyan-400/10 blur-3xl"></div>
        <div class="relative flex h-20 shrink-0 items-center border-b border-white/[0.08] px-5">
            <a href="{{ route('dashboard') }}" class="group flex min-w-0 flex-1 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
                <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-cyan-300 to-cyan-500 text-slate-950 shadow-lg shadow-cyan-950/40 transition group-hover:scale-105">
        ...
        <div class="relative shrink-0 border-t border-white/[0.08] bg-black/10 p-4">
            <div class="flex items-center gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.04] p-3">
                <span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-indigo-400/15 text-sm font-black text-indigo-200 ring-1 ring-inset ring-indigo-300/20">
                    {{ mb_strtoupper(mb_substr(auth()->user()->name, 0, 1)) }}
                </span>
                <a href="{{ route('profile.edit') }}" class="min-w-0 flex-1 rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
    ...
    <div class="relative min-w-0 flex-1 overflow-hidden">
        <div class="pointer-events-none absolute inset-x-0 top-0 h-80 bg-[radial-gradient(circle_at_top_right,_rgba(34,211,238,0.09),_transparent_45%),radial-gradient(circle_at_top_left,_rgba(129,140,248,0.07),_transparent_40%)]"></div>
        <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/85 px-4 shadow-sm shadow-slate-200/30 backdrop-blur-xl lg:hidden">
            <button type="button" @click="menu = true" class="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm transition hover:border-cyan-300 hover:text-cyan-700" aria-controls="menu-lateral" :aria-expanded="menu" aria-label="Abrir menu">
                <x-icon name="menu" class="h-5 w-5" />
            </button>
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2 font-black tracking-tight text-slate-950">
                <span class="grid h-8 w-8 place-items-center rounded-lg bg-cyan-400 text-slate-950"><x-icon name="clock" class="h-[18px] w-[18px]" /></span>
                Tempo Interno
            </a>
            <a href="{{ route('profile.edit') }}" class="grid h-10 w-10 place-items-center rounded-xl bg-slate-900 text-sm font-black text-white" aria-label="Abrir perfil">
                {{ mb_strtoupper(mb_substr(auth()->user()->name, 0, 1)) }}
            </a>
        </header>
        ...
        <main id="conteudo-principal" class="relative mx-auto min-h-screen w-full max-w-[1600px] p-4 sm:p-6 lg:p-8 xl:p-10">
            @if(isset($header))
                <div class="mb-7 border-b border-slate-200/80 pb-6">{{ $header }}</div>
            @endif
            <x-flash-messages />
            {{ $slot }}
        </main>
