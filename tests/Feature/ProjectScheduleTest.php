...
class ProjectScheduleTest extends TestCase
{
    ...
    public function test_user_can_open_the_spreadsheet_for_a_project(): void
    {
        ...
        $this->actingAs($this->user)
            ...
            ->assertSee('aria-label="Adicionar coluna"', false)
            ->assertSee('placeholder="Nome da coluna"', false)
            ->assertDontSee('A nova coluna será adicionada ao final')
            ->assertSee('Atualizar usando uma planilha Excel')
            ->assertSee('x-data="appShell"', false)
            ->assertSee('id="confirmation-modal-title"', false)
            ->assertSee('data-confirm-title="Importar planilha?"', false)
